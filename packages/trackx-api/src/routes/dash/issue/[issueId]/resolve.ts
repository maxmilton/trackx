...
const setIssueDoneStmt = db.prepare('UPDATE issue SET done = ? WHERE id = ?');

function setIssueDone(issueId: string, state: boolean) {
  return setIssueDoneStmt.run(state ? 1 : 0, issueId);
}
